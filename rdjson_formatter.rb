...
class RdjsonFormatter < RuboCop::Formatter::BaseFormatter
  ...
  def finished(_inspected_files)
    puts @rdjson.to_json # rubocop:disable Rails/Output
  end
  ...
  def build_suggestions(offense)
    action, range, content, _ = offense.corrector.as_nested_actions[0]
    if action == :replace && offense.source_line.strip == '' && content == ''
      text = ''
    else
      line_span = (range.line_span.begin - 1)..(range.line_span.end - 1)
      text = offense.corrector.process.lines[line_span].join('')
      if text[-1] == "\n"
        text = text[0..-2]
      end
    end
